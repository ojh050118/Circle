...
void main(void)
{
	...
	for (int i = 0; i < 80; i++)
	{
		...
		u_xlat4 = toSRGB(texture2D(m_Sampler, u_xlat13.xy));
		u_xlat3.z = u_xlat4.x * 0.2;
		u_xlat18 += u_xlat4.x * 0.2;
		u_xlat2.xyz = u_xlat1.y < u_xlat18 ? u_xlat3.xyz : u_xlat2.xyz;
	}

	u_xlat1 = toSRGB(texture2D(m_Sampler, u_xlat2.xy * vec2(0.4)));
	u_xlat3 = (u_xlat2.xyxy + vec4(0.0, 0.004, 0.0, -0.004)) * vec4(0.4);
	u_xlat4 = toSRGB(texture2D(m_Sampler, u_xlat3.xy));
	u_xlat3 = toSRGB(texture2D(m_Sampler, u_xlat3.zw));
	u_xlat3.y = max(u_xlat4.x * 0.2 - u_xlat3.x * 0.2, 0.003);
	u_xlat4 = (u_xlat2.xyxy + vec4(0.004, 0.0, -0.004, 0.0)) * vec4(0.4);
	u_xlat3.x = max(toSRGB(texture2D(m_Sampler, u_xlat4.xy)).x * 0.2 - toSRGB(texture2D(m_Sampler, u_xlat4.zw)).x * 0.2, 0.003);
	...
	gl_FragColor = vec4(u_xlat1.xyz * vec3(1.0 - u_xlat0.x) + vec3(u_xlat0.x * 0.8), 1.0);
}